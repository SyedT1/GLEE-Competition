...
# The script is distributed as <package>/analysis/analyze_v33_transcripts.py.
# Resolve every input relative to that package root so a clean extraction works
# regardless of the directory chosen by the user.
PAPER_ROOT = SCRIPT_DIR.parent
ROOT = PAPER_ROOT
PAPER_DATA = PAPER_ROOT / "data"
PAPER_FIGURES = PAPER_ROOT / "figures"
PAPER_TABLES = PAPER_ROOT / "tables"
TRANSCRIPT_PATH = PAPER_DATA / "all_game_transcripts_v33.txt"
RATING_PATH = PAPER_DATA / "v33_rating_outcomes.csv"
SESSION_REPORT_PATH = PAPER_ROOT / "artifacts" / "v33" / "glee_v33_session_report.json"
EVIDENCE_PATH = PAPER_ROOT / "artifacts" / "v33" / "glee_v33_evidence.jsonl"
...
def execution_activation_audit(
    rating_rows: list[dict[str, object]],
) -> tuple[list[dict[str, object]], dict[str, object]]:
    """Count executed horizon branches and safeguards for the 248 live IDs."""
    live_ids = {str(row["game_id"]) for row in rating_rows}
    ...
    live_events = [event for event in events if str(event.get("payload", {}).get("game_id", "")) in live_ids]

    v33_decisions = [event for event in live_events if event["event"] == "v33_heuristic_decision"]
    contract_fallbacks = [
        event for event in live_events
        if event["event"] == "decision" and event["payload"].get("used_fallback")
    ]
    reasons: defaultdict[str, list[dict[str, object]]] = defaultdict(list)
    for event in live_events:
        if event["event"] not in {"v27_context_repair_decision", "role_isolated_recovery_decision"}:
            continue
        reason = event["payload"].get("metrics", {}).get("reason")
        if reason:
            reasons[str(reason)].append(event)

    unknown_ids: dict[str, set[str]] = {family: set() for family in FAMILY_ORDER}
    for row in rating_rows:
        family = str(row["family"])
        state = assignments[str(row["game_id"])]["state"]
        if family == "persuasion":
            unknown = state.get("total_rounds") in {None, 0, ""}
        else:
            unknown = state.get("horizon_known") is False or state.get("max_rounds") in {None, 0, ""}
        if unknown:
            unknown_ids[family].add(str(row["game_id"]))

    unknown_decisions = {
        family: sum(
            str(event["payload"]["game_id"]) in game_ids
            for event in v33_decisions
        )
        for family, game_ids in unknown_ids.items()
    }
    bob_events = reasons["bob_delay_aware_offer"] + reasons["bob_delay_aware_accept"]
    cycle_events = reasons["bob_bounded_cycle_match"] + reasons["infeasible_cycle"]
    repair_events = reasons["weak_context_midpoint_offer"] + reasons["weak_context_faster_counter"]
    missing_horizon_repairs = [
        event for event in repair_events
        if str(event["payload"]["game_id"]) in unknown_ids["negotiation"]
    ]
    checkpoints = [event for event in events if event["event"] == "v33_microbatch_checkpoint"]
    family_summaries = [event for event in events if event["event"] == "v33_family_summary"]
    drawdown_stops = [
        event for event in family_summaries
        if "drawdown" in str(event["payload"].get("stop_reason", "")).lower()
    ]
    overshoots = [event for event in checkpoints if int(event["payload"].get("assignment_overshoot", 0)) > 0]

    def row(label: str, selected: list[dict[str, object]], detail: str) -> dict[str, object]:
        return {
            "measure": label,
            "games_or_families": len({
                str(event["payload"].get("game_id", event["payload"].get("family", "")))
    ...
    audit_rows = [
        row("Contract fallback", contract_fallbacks, f"of {len(v33_decisions)} live decisions"),
        row("Bob recovery override", bob_events, "29 offers; 17 acceptance overrides"),
        row("Cycle intervention", cycle_events, "1 Bob bounded match; 2 negotiation walkaways"),
        row("Seller short-horizon repair", repair_events,
            f"{len(missing_horizon_repairs)} activations came from one unknown-horizon game"),
        row("Family drawdown stop", drawdown_stops, "persuasion and negotiation"),
        row("Assignment overshoot", overshoots, f"across {len(checkpoints)} checkpoints"),
        {
            "measure": "Telemetry abort",
            "games_or_families": 0,
            "activations": 0,
            "detail": f"{len(report.get('telemetry', []))} errors arrived after checkpoint checks",
        },
    ]
    audit_summary: dict[str, object] = {
        "live_policy_decisions": len(v33_decisions),
        "unknown_horizon_games": {family: len(ids) for family, ids in unknown_ids.items()},
        "unknown_horizon_decisions": unknown_decisions,
        "missing_horizon_short_repair_games": len({str(event["payload"]["game_id"]) for event in missing_horizon_repairs}),
        "missing_horizon_short_repair_decisions": len(missing_horizon_repairs),
        "telemetry_errors_recorded_after_checkpoint": len(report.get("telemetry", [])),
        "telemetry_abort_activations": sum(bool(event["payload"].get("global_abort")) for event in family_summaries),
    }
...
def write_generated_tables(family_stats: list[dict[str, object]], role_stats: list[dict[str, object]],
                           persuasion_support: list[dict[str, object]], transcript_rows: list[dict[str, object]],
                           sensitivity: list[dict[str, object]]) -> None:
    lines = [
        r"\begin{table}[t]", r"\caption{Descriptive distribution of assignment-level displayed rating changes. No inferential interval is reported because matchmaking, opponent repetition, drift, and stopping are uncontrolled.}",
        r"\label{tab:v33-distribution}", r"\centering\scriptsize",
        r"\begin{tabular}{lrrrrr}", r"\toprule",
        r"Family & $+ / 0 / -$ & Mean & Median & SD & IQR \\", r"\midrule",
    ]
    for row in family_stats:
        signs = f"{row['positive']}/{row['zero']}/{row['negative']}"
        lines.append(f"{str(row['family']).title()} & {signs} & {num2(row['mean'])} & {num2(row['median'])} & {num2(row['sd'])} & {num2(row['iqr'])} " + r"\\")
    lines += [r"\bottomrule", r"\end{tabular}", r"\end{table}", ""]

    lines += [
        r"\begin{table}[t]", r"\caption{Persuasion role--mode--information support. Hidden-value assignments contain no V33 buyers, so hidden/known marginal differences are not role-adjusted comparisons.}",
        r"\label{tab:v33-persuasion-support}", r"\centering\scriptsize", r"\begin{tabular}{llllrrr}", r"\toprule",
        r"Role & Mode & Values & $n$ & Sum & Mean & $+ / 0 / -$ \\", r"\midrule",
    ]
    for row in persuasion_support:
        signs = f"{row['positive']}/{row['zero']}/{row['negative']}"
        lines.append(f"{fmt(str(row['role']).title())} & {fmt(row['mode'])} & {fmt(row['values_known'])} & {row['n']} & {num2(row['sum'])} & {num2(row['mean'])} & {signs} " + r"\\")
    lines += [r"\bottomrule", r"\end{tabular}", r"\end{table}", ""]
    ...
    reason_labels = {
        "role_loss:player_2": "Bob loss",
        "role_drawdown:player_2": "Bob DD",
        "role_loss:buyer": "buyer loss",
        "role_drawdown:buyer": "buyer DD",
        "family_loss": "family loss",
        "family_drawdown": "family DD",
        "target": "target",
        "observed_end": "end",
    }
    for family in FAMILY_ORDER:
        ...
        for scale in (0.50, 0.75, 1.00, 1.25, 1.50):
            ...
            reason = reason_labels.get(str(row["reason"]), str(row["reason"]))
            cells.append(f"{row['stop_n']} ({fmt(reason)})")
...
def plot_sequences(rows: list[dict[str, object]]) -> None:
    ...
    pdf_metadata = {
        "Creator": "GLEE V33 audit generator",
        "Producer": "Matplotlib",
        "CreationDate": None,
        "ModDate": None,
    }
    figure.savefig(
        PAPER_FIGURES / "v33_sequential_ratings.pdf",
        bbox_inches="tight",
...
def main() -> None:
    ...
    persuasion_support = grouped_statistics(
        [row for row in rating_rows if row["family"] == "persuasion"],
        ("family", "role", "mode", "values_known"),
    )
    transcript_config_stats = transcript_configuration_statistics(transcript_rows)
    activation_rows, activation_summary = execution_activation_audit(rating_rows)
    ...
    write_csv(PAPER_DATA / "v33_rating_outcomes_sequenced.csv", rating_rows)
    ...
    write_csv(PAPER_DATA / "v33_execution_activation_audit.csv", activation_rows)
    write_generated_tables(family_stats, role_stats, persuasion_support, transcript_rows, sensitivity)
    ...
    summary = {
        ...
        "execution_activation_audit": activation_summary,
    }
