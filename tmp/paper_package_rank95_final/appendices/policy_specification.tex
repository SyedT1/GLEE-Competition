...
the archived notebook remains authoritative. The definitions below also state
the parsing, update, tie-breaking, tolerance, and rounding choices needed to
map visible states to actions.
...
Offer and price outputs are rounded to eight decimals. Bargaining threshold
comparisons use a $10^{-12}$ slack and negotiation profitability/threshold
comparisons use $10^{-9}$.
...
stall count, and $\delta_i$ own discount. With visible discounts, the
configuration prior is
$b_0=1-(1-\delta_j)/(1-\delta_i\delta_j)$, clipped to $[.001,.999]$; it is
$.5$ when the denominator is at most $10^{-12}$
\citep{rubinstein1982perfect}. Otherwise it is $0.46+0.02t$ against a disclosed human and
$0.43+0.02t$ otherwise. If $z$ is the largest opponent share in a rejected own
proposal (zero before one exists) and $m$ the median of the five latest demands,
the base floor used
below is $b=\operatorname{clip}(\max\{b_0,z+.006,m-.065+.020t\},.20,.999)$,
omitting unavailable terms. In the role formulas below,
$f_{\rm reject}=z$.
...
\begin{enumerate}
 ...
 with $\gamma=1.18$. Candidate tuples $(\text{score},1-y,y)$ are compared
 lexicographically, so an exact score tie retains more for the proposer. The
 responder amount is rounded to eight decimals and the proposer receives the
 exact residual.
...
\]
where the favorable opponent concession is minus the median adjacent step of
the recent demand path and
$g=\operatorname{clip}(.25\times\text{favorable opponent concession},
...
negative, or unknown. The parser lowercases text, converts underscores to
spaces, removes non-alphanumeric punctuation, and collapses whitespace. Exact
negative forms are \{no, false, negative, not recommended\}; negative phrases
are \{do not buy, don't buy, should not buy, shouldn't buy, do not recommend,
don't recommend, recommend against, not worth, not a good, not high quality,
low quality, bad product, buying would be foolish\}, and any of \{pass, skip,
avoid\} is negative. These checks precede exact positive forms \{yes, true,
recommended, buy\} and phrases \{buy this, recommend buying, recommend this,
worth it, high quality, great product, good product, positive\}; unmatched text
is unknown.
For a positive signal use $q_H=.90,q_L=.24$; for a
...
$\hat p v+(1-\hat p)u+B\ge P$. Here $\hat p$ is only a pseudo-count reliability
score: named-opponent memory may pool purchased labels across different priors
and prices, so no exchangeable Bayesian model is asserted. The bonus
$B=.010(v-u)$ times the remaining-round
...
``persuasion buyer V23'' denotes the purchased-only pseudo-count reliability rule.
