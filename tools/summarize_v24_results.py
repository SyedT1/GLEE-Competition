"""Create paper-ready, live-only V24 result tables from saved artifacts."""
...
ARTIFACT_DIR = ROOT / "notebooks" / "Glee_competition_24"
REPORT_PATH = ARTIFACT_DIR / "glee_v24_session_report.json"
EVIDENCE_PATH = ARTIFACT_DIR / "glee_v24_evidence.jsonl"
...
def family_summaries() -> list[dict]:
    ...
    with EVIDENCE_PATH.open(encoding="utf-8") as handle:
        for line in handle:
            ...
            if event.get("event") == "v24_family_summary":
                rows.append(event["payload"])
    if {row["family"] for row in rows} != {"bargaining", "persuasion"}:
        raise RuntimeError("Expected V24 bargaining and persuasion summaries")
    return rows
...
def main() -> None:
    ...
    if len(outcomes) != 20:
        raise RuntimeError(f"Expected 20 live outcomes, found {len(outcomes)}")
    if report["action_fallbacks"] or report["telemetry"]:
        raise RuntimeError("V24 contains a fallback or telemetry diagnostic")

    DATA_DIR.mkdir(parents=True, exist_ok=True)
    games_path = DATA_DIR / "v24_live_rating_games.csv"
    ...
    aggregate_path = DATA_DIR / "v24_live_rating_aggregate.csv"
    ...
    summaries_path = DATA_DIR / "v24_family_summaries.csv"
