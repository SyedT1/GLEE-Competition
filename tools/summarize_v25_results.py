"""Create paper-ready, live-only V25 result tables from saved artifacts."""
...
ARTIFACT_DIR = ROOT / "notebooks" / "Glee_competition_25"
REPORT_PATH = ARTIFACT_DIR / "glee_v25_session_report.json"
EVIDENCE_PATH = ARTIFACT_DIR / "glee_v25_evidence.jsonl"
...
def family_summaries() -> list[dict]:
    ...
    with EVIDENCE_PATH.open(encoding="utf-8") as handle:
        for line in handle:
            ...
            if event.get("event") == "v25_family_summary":
                rows.append(event["payload"])
    if {row["family"] for row in rows} != {"bargaining", "negotiation"}:
        raise RuntimeError("Expected V25 bargaining and negotiation summaries")
    return rows
...
def main() -> None:
    ...
    live_ids = {
        game_id for game_id, row in assignments.items()
    ...
    if len(outcomes) != 51:
        raise RuntimeError(f"Expected 51 live outcomes, found {len(outcomes)}")
    if report["action_fallbacks"] or report["telemetry"]:
        raise RuntimeError("V25 contains a fallback or telemetry diagnostic")

    DATA_DIR.mkdir(parents=True, exist_ok=True)
    games_path = DATA_DIR / "v25_live_rating_games.csv"
    ...
    aggregate_path = DATA_DIR / "v25_live_rating_aggregate.csv"
    ...
    summaries_path = DATA_DIR / "v25_family_summaries.csv"
